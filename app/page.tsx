...
export default function HomePage() {
  ...
  const trendingStyles = [
    { name: "Cyberpunk", growth: "+24%", image: "/cyberpunk-style.png" },
    { name: "Anime", growth: "+18%", image: "/anime-style.png" },
    { name: "Vaporwave", growth: "+15%", image: "/vaporwave-style.png" },
    { name: "Pixel Art", growth: "+12%", image: "/pixel-art-style.png" },
  ]

  // Mock data for latest creations
  const latestCreations = Array.from({ length: 6 }, (_, i) => ({
    id: i + 1,
    title: `${["Cosmic", "Digital", "Neon", "Abstract", "Pixel", "Surreal"][i % 6]} ${["Dreams", "Visions", "Landscape", "Portrait", "World", "Dimension"][i % 6]} #${i + 1}`,
    price: (Math.random() * 100 + 20).toFixed(0),
    image: `/abstract-geometric-flow.png?height=400&width=400&query=digital art style ${i}`,
  }))
        ...
        <div className="container px-4 md:px-6 mx-auto flex flex-col h-full justify-center">
          <div className="text-center mb-8">
            ...
            <p className="max-w-[700px] mx-auto text-slate-600 md:text-xl">
              Create, remix, and drop viral‑ready memes and mini‑clips before the trend cools off
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {trendingStyles.map((style, index) => (
              <Link href={`/create?style=${style.name.toLowerCase()}`} key={index} className="group">
                <div
                ...
                >
                  <div className="aspect-square relative">
                    ...
                    <Image
                      src={style.image || `/placeholder.svg?height=300&width=300&query=${style.name} style art`}
                      alt={style.name}
                      fill
      ...
      <section className="w-full py-12 bg-slate-50">
        <div className="container px-4 md:px-6 mx-auto">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold font-display">Latest Creations</h2>
              ...
              <Card key={artwork.id} className="overflow-hidden border-slate-200 transition-all card-hover">
                <div className="aspect-square relative">
                  <Image
                    src={
                      artwork.image ||
                      `/placeholder.svg?height=400&width=400&query=${encodeURIComponent(artwork.title) || "/placeholder.svg"}`
                    }
                    alt={artwork.title}
                    fill
